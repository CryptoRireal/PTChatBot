...
class ProfitTrailer::API
  class << self
    ...
    def fetch_data_profit
      ...
      {
        market: data[:market],
        profit_today_btc: number_to_rounded(data[:totalProfitToday], precision: 8),
        profit_today_pct: number_to_percentage(data[:totalProfitToday] / total_value * 100, precision: 2),
        profit_week_btc: number_to_rounded(data[:totalProfitWeek], precision: 8),
        profit_week_pct: number_to_percentage(data[:totalProfitWeek] / total_value * 100, precision: 2),
        profit_yesterday_btc: number_to_rounded(data[:totalProfitYesterday], precision: 8),
        profit_yesterday_pct: number_to_percentage(data[:totalProfitYesterday] / total_value * 100, precision: 2),
        total_value_btc: number_to_rounded(total_value, precision: 8),
    ...
    def fetch_data_pairs
      ...
      (data[:gainLogData] || []).map do |pair|
...

        {
          ...
          average_price_btc: number_to_rounded(average_calc[:avgPrice], precision: 8),
          current_price_btc: number_to_rounded(pair[:currentPrice], precision: 8),
          ...
          total_amount: number_to_rounded(average_calc[:totalAmount], precision: 8),
    ...
    def fetch_data_dca
      ...
      (data[:dcaLogData] || []).map do |dca|
...

        {
          ...
          current_price_btc: number_to_rounded(dca[:currentPrice], precision: 8),
          current_price_usd: number_to_currency(dca[:currentPrice] * data[:BTCUSDTPrice]),
          date: Date.parse(first_bought[:date].values.join("-")).to_s,
          dca_count: dca[:boughtTimes],
          estimated_value_btc: number_to_rounded(average_calc[:totalAmount] * dca[:currentPrice], precision: 8),
    ...
    def conn
      @_conn ||= begin
        Faraday.new(url: ProfitTrailer.config["PT_WEB_URL"]) do |faraday|
          faraday.use :cookie_jar
          faraday.adapter Faraday.default_adapter
        end.
        tap do |c|
          c.post("/login?password=#{ProfitTrailer.config["PT_WEB_PASSWORD"]}")
        end
      end
    end
